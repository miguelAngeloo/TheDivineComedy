...
export class Spawn implements System {
  ...
  async initAsync(ctx: SystemContext): Promise<void> {
    ...
    const listenerFn = (userId: number, userProfileId: number, discordRoleIds: string[], discordId?: string) => {
      ...
      // const listenerFn = (userId, userProfileId, discordRoleIds, discordId) => {
      // if the gamemode provides a hook, it is IT that decides the spawn.
      const gamemodeHook = (ctx.svr as any).onPlayerSpawn;

      if (typeof gamemodeHook === "function") {
        gamemodeHook(userId, userProfileId);
        return;
      }
